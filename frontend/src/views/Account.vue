...
const currentWallpaper = currentURL.length == 36 ?  await wallpaperStore.getWallpaperById(currentURL) : undefined;
...
<template>
  ...
  <div v-if="userStore.loggedIn" class="flex flex-col items-center">
    <h1>Hi! You are:</h1>
    <span>{{ userStore.user.name }} with email: {{ userStore.user.email }}</span>
<br />
    Actual wallpaper: <br/>
    <Wallpaper :wallpaper="currentWallpaper" v-if="currentWallpaper"></Wallpaper>
  </div>
</template>
