...
const data = computed(() => {
    return wallpaperStore.wallpapers.filter((wallpaper) => {
        return wallpaper.name.toLowerCase().includes(searchDebounced.value.toLowerCase())
    })
})
...
<template>
    <Drop text="Drop your pictures here" @drop='onDrop'>
    </Drop>
...
<a-flex justify="center">
 ...
 :loading="!wallpaperStore.isFetched">
    <template #renderItem="{ item }">
      <a-list-item>
            <SingleWallpaper :wallpaper="item"
            ></SingleWallpaper>
      </a-list-item>
    </template>
